...
impl StructuralFindAndReplace {
    ...
    pub fn visit_tag_sequence(&mut self, text: &mut Vec<Segment>) {
        ...
        let find_leading_literal = match &self.find[0] {
            Segment::Literal(value) => Some(value),
            _ => None,
        };
        let find_trailing_literal = match &self.find[self.find.len() - 1] {
            Segment::Literal(value) => Some(value),
            _ => None,
        };
        if find_leading_literal.is_some() && find_trailing_literal.is_some() && self.find.len() == 1
        {
            ...
            let find_string = match &self.find[0] {
            ...
            for i in (0..text.len()).rev() {
                let segment = &text[i];
                if segment == &self.find[0] {
                    text.splice(i..=i, self.replace.iter().cloned());
                }
            }
        } else {
            ...
            for i in (0..text.len() + 1 - self.find.len()).rev() {
                ...
                if matches {
                    for (original_segment, find_segment) in text[i + 1..i + self.find.len() - 1]
                        .iter()
                        .zip(self.find[1..self.find.len() - 1].iter())
                    {
                ...
                if matches {
                    ...
                    result.extend(self.replace.iter().cloned());
                    if let (Some(find_trailing_literal), Segment::Literal(text_trailing_literal)) =
                        (find_trailing_literal, &text[i + self.find.len() - 1])
                    {
                        if text_trailing_literal.len() > find_trailing_literal.len() {
                            result.push(Segment::Literal(
                                text_trailing_literal[find_trailing_literal.len()..].to_string(),
                            ));
                        }
                    }
...
mod tests {
    ...
    fn replace_substring_with_text() {
        let mut visitor = StructuralFindAndReplace {
